...
class BaseballGame {
    ...
    func checkStrikeBall(answer: [Int], input: String) -> Bool {
        ...
        var ball = 0
        for i in 0...9 {
            if let answerIndex = answer.firstIndex(of: i), // example: 9 index 2
               let inputIndex = input.firstIndex(of: i) { // if let은 nil값이 아닐때만 실행함(optional binding)
    ...
    func makeAnswer() -> [Int] {
        ...
        answer.append(Int.random(in: 1...9))
        answer.append(Int.random(in: 1...9))
        if Set(answer).count == 3 {   // set을 왜썼을까?
            return answer
        } else {
            return makeAnswer() // 이새끼는 뭘까(recursive, 재귀)
        }
    }
}
